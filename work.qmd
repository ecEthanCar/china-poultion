...
all_good_property_for_cleaned <- property_for_cleaned |> 
  filter(Province.x == Province.y | !is.na(Statistical_Division_Code))
wierd_property_for_cleaned <- property_for_cleaned |> 
  filter(Province.x != Province.y | is.na(Statistical_Division_Code)) # Just fix manually later
...
pivot_property <- property |>
  pivot_longer(
    cols = c(`1999`:`2021`),
    names_to = "year",
    values_to = "pct"
  ) |>
  select(Province, Region, Subnational, year, pct) |>
  mutate(year = as.numeric(year),
         pct = as.numeric(pct))

pivot_estate <- real_estate |>
  pivot_longer(
    cols = c(`1999`:`2021`),
    names_to = "year",
    values_to = "pct"
